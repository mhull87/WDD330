...
export default class QuakesController {
  ...
  async getQuakesByRadius(radius = 100) {
    //this method provides the glue between the model and view notice it first goes out and requests the approriate data from the model, then it passes it to the view to be rendered.
     //set loading message
     ...
     //add a listener to the new list of quakes to allow drill down into the details
     this.parentElement.addEventListiner('touchend', e => {
       this.getQuakeDetails(e.target.dataset.id); 
     });
  }
  async getQuakeDetails(quakeId) {
       //get the details for the quakeId provided from the model, then send them to the view to be displayed
    this.quakesView.renderQuake(quakeId, element);
  }
}